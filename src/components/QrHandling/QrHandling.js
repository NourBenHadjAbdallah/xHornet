  ...
  const storeDiplomaToBlockchain = async () => {
    try {
      ...
      const sanitizeInput = (value) => {
        if (typeof value !== 'string') throw new Error(`Invalid input type for ${value}`);
        return value.trim().replace(/\s+/g, " "); // Remove extra spaces and normalize
      };

      const plainDiplomaData = {
        fullName: sanitizeInput(`${lastName} ${firstName}`),
        degree: sanitizeInput(Diploma),
        specialty: sanitizeInput(speciality),
        mention: sanitizeInput(mention),
        idNumber: sanitizeInput(id),
        academicYear: sanitizeInput(academicFullYear),
        juryMeetingDate: sanitizeInput(dateProces),
        directorName: sanitizeInput(configData.DIRECTEUR)
      };

      // Validate required fields
      const requiredFields = ['fullName', 'degree', 'specialty', 'mention', 'idNumber', 'academicYear', 'juryMeetingDate', 'directorName'];
      for (const field of requiredFields) {
        if (!plainDiplomaData[field] || plainDiplomaData[field].length === 0) {
          throw new Error(`Missing or empty required field: ${field}`);
        }
      }

      // Generate hash using validated data
      const onChainDiplomaHash = generateDiplomaHash(plainDiplomaData);
      console.log("Generated diploma hash (for on-chain and PDF):", onChainDiplomaHash);
