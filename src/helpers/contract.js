import { ethers } from "ethers";
import { connectWalletWithPrivateKey } from './wallet.js';
import CONTRACT_ABI from './ABI.json';


//const CONTRACT_ADDRESS = "0x7Dba8948a8d6E5CABF907fA4cAd498d4e49d069d";
//const CONTRACT_ADDRESS = "0x732aDB73A60D3f2dED37517b34b599787d640f53";
// const CONTRACT_ADDRESS = "0xe97a27B2ec34852333dD96a8927a11fE2d4ce912";
//const CONTRACT_ADDRESS = "0xa0F54a6d385aBdCa20c12A80d4d12072C4E8590b";
// const CONTRACT_ADDRESS = "0xCcdd3740eA3bFb31c5a9DC01fcb9647e5AF6edCe"; //ETH
//const CONTRACT_ADDRESS = "0xf647Cb668D021210c7bE40Eefa8A511f8C27966D"; //BSC
const CONTRACT_ADDRESS = "0xc1Fbcba0163Bef2882Ff3F622144e07006F33Ede"; //POL

/*export const connectToContract = async (privateKey) => {
  try {
    if (!ethers.utils.isAddress(CONTRACT_ADDRESS)) {
      throw new Error("❌ Invalid contract address");
    }

    const walletConnection = await connectWalletWithPrivateKey(privateKey);

    const contract = new ethers.Contract(
      CONTRACT_ADDRESS,
      CONTRACT_ABI,
      walletConnection.signer
    );

    console.log("📜 Connected to Verif contract");
    return { contract, walletConnection };
  } catch (error) {
    console.error("❌ Failed to connect to contract:", error.message);
    throw error;
  }
};

export const issueDiploma = async (contractConnection, Diploma) => {
  try {
    const {
      fullName,
      degree,
      specialty,
      mention,
      idNumber,
      academicYear,
      juryMeetingDate,
      directorName,
      diplomaHash
    } = Diploma;

    console.log("📋 Diploma data:", { fullName, degree, specialty, mention, idNumber, academicYear, juryMeetingDate, diplomaHash, directorName  });

    const tx = await contractConnection.contract.issueDiploma(
      diplomaHash,
      fullName,
      degree,
      specialty,
      mention,
      idNumber,
      academicYear,
      juryMeetingDate,
      directorName
    );

    const receipt = await tx.wait();
    return {
      success: true,
      txHash: tx.hash,
      diplomaHash,
      blockNumber: receipt.blockNumber,
      gasUsed: receipt.gasUsed.toString()
    };
  } catch (error) {
    if (error.code === 'CALL_EXCEPTION') {
      console.error("💥 Contract reverted:", error.reason || "Unknown reason");
      if (error.reason?.includes("DiplomaRegistry: Diploma hash already exists")) {
        console.error("🚫 This diploma has already been issued");
      } else if (error.reason?.includes("AccessControl")) {
        console.error("🚫 Access denied: Caller does not have the ISSUER role.");
      }
    }
    console.error("❌ Failed to issue diploma:", error.message);
    throw error;
  }
};

export const storeDiplomasBatch = async (contractConnection, diplomaInputs) => {
  try {
    // Validate input is an array of objects with correct fields
    if (!Array.isArray(diplomaInputs) || diplomaInputs.length === 0) {
      throw new Error("Diploma batch must be a non-empty array.");
    }
    // Each object must have all required fields
    for (const d of diplomaInputs) {
      if (
        !d.diplomaHash ||
        !d.fullName ||
        !d.degree ||
        !d.specialty ||
        !d.mention ||
        !d.idNumber ||
        !d.academicYear ||
        !d.juryMeetingDate ||
        !d.directorName
      ) {
        throw new Error("Each diploma input must have all required fields.");
      }
    }

    // Call the contract with the array
    const tx = await contractConnection.contract.storeDiplomasBatch(diplomaInputs);
    const receipt = await tx.wait();
    return {
      success: true,
      txHash: tx.hash,
      blockNumber: receipt.blockNumber,
      gasUsed: receipt.gasUsed.toString(),
    };
  } catch (error) {
    if(error.code === 'CALL_EXCEPTION') {
      console.error("💥 Contract reverted:", error.reason || "Unknown reason");
      if (error.reason?.includes("Duplicate diploma")){
        console.error("🚫 One or more diplomas in the batch already exist.");
      }
    }
    console.error("❌ Failed to batch issue diplomas:", error.message);
    throw error;
  }
};*/

export const connectToContract = async (privateKey) => {
  try {
    if (!ethers.utils.isAddress(CONTRACT_ADDRESS)) {
      throw new Error("❌ Invalid contract address");
    }

    const walletConnection = await connectWalletWithPrivateKey(privateKey);
    const signerAddress = await walletConnection.signer.getAddress();
    console.log("Signer address:", signerAddress);

    // Verify network
    const provider = walletConnection.signer.provider;
    const network = await provider.getNetwork();
    console.log("Connected network:", network.name, network.chainId);  // Should be chainId: 80002 for Amoy

    // Verify balance
    const balance = await walletConnection.signer.getBalance();
    console.log("Signer balance:", ethers.utils.formatEther(balance), "MATIC");

    const contract = new ethers.Contract(
      CONTRACT_ADDRESS,
      CONTRACT_ABI,
      walletConnection.signer
    );

    // Check ISSUER role
    const ISSUER_ROLE = ethers.utils.keccak256(ethers.utils.toUtf8Bytes("ISSUER"));
    const hasIssuerRole = await contract.hasRole(ISSUER_ROLE, signerAddress);
    console.log(`Does ${signerAddress} have ISSUER role?`, hasIssuerRole);

    // Check DEFAULT_ADMIN_ROLE (optional, for debugging)
    const DEFAULT_ADMIN_ROLE = ethers.constants.HashZero;
    const hasAdminRole = await contract.hasRole(DEFAULT_ADMIN_ROLE, signerAddress);
    console.log(`Does ${signerAddress} have DEFAULT_ADMIN_ROLE?`, hasAdminRole);

    if (!hasIssuerRole) {
      throw new Error(`Signer ${signerAddress} does not have ISSUER role`);
    }

    console.log("📜 Connected to Verif contract");
    return { contract, walletConnection };
  } catch (error) {
    console.error("❌ Failed to connect to contract:", error.message);
    throw error;
  }
};

export const issueDiploma = async (contractConnection, Diploma) => {
  try {
    const {
      fullName = "Test Name",
      degree = "Test Degree",
      specialty = "Test Specialty",
      mention = "Test Mention",
      idNumber = "12345",
      academicYear = "2025",
      juryMeetingDate = "2025-09-23",
      directorName = "Test Director",
      diplomaHash = ethers.utils.keccak256(ethers.utils.toUtf8Bytes("test-diploma-123"))
    } = Diploma || {};  // Fallback to defaults if Diploma is undefined

    console.log("📋 Diploma data:", { fullName, degree, specialty, mention, idNumber, academicYear, juryMeetingDate, diplomaHash, directorName });

    // Estimate gas
    const gasEstimate = await contractConnection.contract.estimateGas.issueDiploma(
      diplomaHash,
      fullName,
      degree,
      specialty,
      mention,
      idNumber,
      academicYear,
      juryMeetingDate,
      directorName
    );
    console.log("Estimated gas:", gasEstimate.toString());

    const tx = await contractConnection.contract.issueDiploma(
      diplomaHash,
      fullName,
      degree,
      specialty,
      mention,
      idNumber,
      academicYear,
      juryMeetingDate,
      directorName,
      { gasLimit: gasEstimate.mul(12).div(10) }  // 20% buffer
    );

    console.log("📤 Transaction sent, hash:", tx.hash);

    // Wait for confirmation with detailed error handling
    const receipt = await tx.wait();
    if (receipt.status === 0) {
      console.error("💥 Transaction reverted, receipt:", receipt);
      throw new Error("Transaction reverted: " + (receipt.logs.length > 0 ? ethers.utils.parseBytes32String(receipt.logs[0].data) : "Unknown reason"));
    }

    console.log("✅ Transaction confirmed in block:", receipt.blockNumber);
    return {
      success: true,
      txHash: tx.hash,
      diplomaHash,
      blockNumber: receipt.blockNumber,
      gasUsed: receipt.gasUsed.toString()
    };
  } catch (error) {
    console.error("❌ Failed to issue diploma:", error);
    if (error.code === 'CALL_EXCEPTION') {
      console.error("💥 Contract reverted:", error.reason || "Unknown reason");
      console.error("Transaction hash:", error.transactionHash || "Not available");
    } else if (error.code === 'UNPREDICTABLE_GAS_LIMIT') {
      console.error("❌ Gas estimation failed:", error.message);
    }
    throw error;
  }
};

export const storeDiplomasBatch = async (contractConnection, diplomaInputs) => {
  try {
    // Validate input is an array of objects with correct fields
    if (!Array.isArray(diplomaInputs) || diplomaInputs.length === 0) {
      throw new Error("Diploma batch must be a non-empty array.");
    }
    // Each object must have all required fields
    for (const d of diplomaInputs) {
      if (
        !d.diplomaHash ||
        !d.fullName ||
        !d.degree ||
        !d.specialty ||
        !d.mention ||
        !d.idNumber ||
        !d.academicYear ||
        !d.juryMeetingDate ||
        !d.directorName
      ) {
        throw new Error("Each diploma input must have all required fields.");
      }
    }

    // Call the contract with the array
    const tx = await contractConnection.contract.storeDiplomasBatch(diplomaInputs);
    const receipt = await tx.wait();
    return {
      success: true,
      txHash: tx.hash,
      blockNumber: receipt.blockNumber,
      gasUsed: receipt.gasUsed.toString(),
    };
  } catch (error) {
    if(error.code === 'CALL_EXCEPTION') {
      console.error("💥 Contract reverted:", error.reason || "Unknown reason");
      if (error.reason?.includes("Duplicate diploma")){
        console.error("🚫 One or more diplomas in the batch already exist.");
      }
    }
    console.error("❌ Failed to batch issue diplomas:", error.message);
    throw error;
  }
};